...
Courses live at `site/courses/<NN>-<slug>/index.html` (the summary page) — `site/`
is the published web root on Cloudflare Pages, so URLs drop the prefix. Interactive
lessons sit under `site/courses/<NN>-<slug>/lessons/`. If a course needs live demos,
it gets ONE Blazor WebAssembly project, `apps-src/Course<NN>/`, published to
`site/courses/<NN>-<slug>/app/`. Use an existing course as a model: summary pages
like `site/courses/07-software-advanced/index.html`, and the Blazor app
`apps-src/Course08/`.
...
2. **Create the summary page.** Copy an existing course's `index.html` (same programme is closest) to `site/courses/<NN>-<slug>/index.html`. Keep the depth-2 relative paths (`../../assets/...`, `../../brand/...`, `../../index.html`, etc. — the page is two levels deep within `site/`). Update: `<title>`, meta description, crumbs, eyebrow, `<h1>`, lead, the `.factbar` (Edition/Dates/Format/Partner), the Overview, and the `.sessions` list. If it will have interactive lessons, add an "Interactive lessons" `<h2>` + `<ol class="sessions">` section (see Course 08).

3. **List it on `site/courses.html`.** Add a `<a class="course" href="courses/<NN>-<slug>/index.html">…</a>` card to the correct programme's `.timeline`, in chronological order. If the course is **currently running**, also update the top "Running now" card to point at it (and give the relevant card a `<span class="pill live">● Currently running</span>`).

4. **Optionally surface on the home page** (`site/index.html`): the "Recently delivered" timeline and, if currently running, the "Running now" card near the top. Keep the stats strip accurate.
